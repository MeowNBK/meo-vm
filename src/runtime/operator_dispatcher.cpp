...
ValueType getValueType(const Value& value) {
    ...
    if (value.is_string()) return ValueType::Str;
    if (value.is_array()) return ValueType::Array;
    if (value.is_hash()) return ValueType::Object;
...
OperatorDispatcher::OperatorDispatcher() {
    ...
    binaryOps[{BIT_AND, VT::Bool, VT::Bool}] = [](const Value& l, const Value& r){ 
        return Value(static_cast<Bool>(static_cast<Int>(l.get<Bool>()) & static_cast<Int>(r.get<Bool>())));
    };
    binaryOps[{BIT_OR,  VT::Bool, VT::Bool}] = [](const Value& l, const Value& r){
        return Value(static_cast<Bool>(static_cast<Int>(l.get<Bool>()) | static_cast<Int>(r.get<Bool>())));
    };
